/**
 * Backend reward calculation utilities
 * This mirrors the frontend implementation for consistency
 */
...
const REWARD_CONFIG = {
  ...
  medium: {
    ...
    tiers: [
      ...
      { minSeconds: 1200, maxSeconds: Infinity, coins: 5, label: "Minimum Reward" },
    ],
  },
  hard: {
    baseCoins: 20,
    tiers: [
      ...
      { minSeconds: 1200, maxSeconds: Infinity, coins: 10, label: "Minimum Reward" },
    ],
  },
};
 ...
 */
function calculateRewardCoins(difficulty, timeSpentSeconds, isCorrect) {
  ...
  if (!config) {
    console.warn(`[RewardCalculation] Unknown difficulty: ${difficulty}, defaulting to easy`);
  ...
  if (!tier) {
    console.warn(`[RewardCalculation] No tier found for ${timeSpentSeconds}s, using last tier`);
